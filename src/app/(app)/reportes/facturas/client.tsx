...
export function ReporteFacturasClient({ data, reportDate, fileName }: ReporteFacturasClientProps) {
  const stats = useMemo(() => {
    ...
    const montoTotal = data.reduce((acc, f) => acc + (f.monto_total || 0), 0);
    const pagadas = data.filter(f => f.estado_factura?.toLowerCase() === 'pagado').length;
    const pendientes = data.filter(f => f.estado_factura?.toLowerCase() === 'pendiente').length;

    return { totalFacturas, montoTotal, pagadas, pendientes };
  }, [data]);
